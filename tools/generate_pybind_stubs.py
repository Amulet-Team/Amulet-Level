...
def main() -> None:
    ...
    amulet_level_path = get_package_dir("amulet.level")
    tests_path = os.path.join(root_path, "tests")
    test_amulet_level_path = os.path.join(tests_path, "test_amulet_level")
    ...
    modules: list[tuple[str, str, str]] = [
        (src_path, amulet_level_path, "amulet.level"),
        (tests_path, test_amulet_level_path, "test_amulet_level"),
    ]
    ...
    for _, module_dir, _ in modules:
        for stub_path in glob.iglob(
            ...
            if os.path.isfile(stub_path[:-1]) and not stub_path.endswith("__init__.pyi"):
                os.remove(stub_path)
