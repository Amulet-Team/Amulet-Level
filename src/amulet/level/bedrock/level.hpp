...
class AMULET_LEVEL_EXPORT BedrockLevel : public Level, public CompactibleLevel, public DiskLevel, public ReloadableLevel {
private:
...
public:
    ...
    ~BedrockLevel() override;

    // Load an existing Bedrock level from the given directory.
    // Thread safe.
    static std::unique_ptr<BedrockLevel> load(const std::filesystem::path&);

    //    // Create a new Bedrock level at the given directory.
    //    // Thread safe.
    //    static std::unique_ptr<BedrockLevel> create(const BedrockCreateArgsV1&);
    ...
    std::shared_ptr<BedrockDimension> get_bedrock_dimension(std::variant<DimensionId, BedrockInternalDimensionID>);
