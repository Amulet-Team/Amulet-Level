...
public:
    ...
    // Thread safe.
    static std::unique_ptr<JavaLevel> load(const std::filesystem::path&);

    // Create a new Java level at the given directory.
    // Thread safe.
    static std::unique_ptr<JavaLevel> create(const JavaCreateArgsV1&);
    ...
    std::shared_ptr<JavaDimension> get_java_dimension(const DimensionId&);
