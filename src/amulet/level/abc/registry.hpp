...
class AMULET_LEVEL_EXPORT IdRegistry {
private:
...
public:
    ...
    NamespacedName numerical_id_to_namespace_id(std::uint32_t index) const;

    // Convert a namespaced id to its numerical id.
    // External shared lock required.
    std::uint32_t namespace_id_to_numerical_id(const NamespacedName& name) const;

    // Register a namespaced id to its numerical id.
    // External unique lock required.
    void register_id(std::uint32_t index, const NamespacedName& name);
    ...
    // External shared lock required.
    const std::map<std::uint32_t, NamespacedName>& ids() const;
};
