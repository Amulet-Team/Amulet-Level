...
class AMULET_LEVEL_EXPORT BedrockDimension : public Dimension {
private:
...
public:
    ...
    // Thread safe.
    std::variant<SelectionBox, SelectionBoxGroup> get_bounds() const override;
    ...
    // Thread safe.
    std::shared_ptr<BedrockChunkHandle> get_bedrock_chunk_handle(std::int64_t cx, std::int64_t cz);
